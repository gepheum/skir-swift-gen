...
class SwiftSourceFileGenerator {
  ...
  private writeCodeForStruct(struct: Record): void {
    ...
    this.push(
      ...
      "private static let _typeAdapter = SkirClient.StructAdapter<",
      selfTypeRef,
      ", _Builder",
      ">(\n",
      "modulePath: ",
      ...
      ");\n\n",
      "fileprivate static func _adapterForInit() -> SkirClient.StructAdapter<",
      selfTypeRef,
      ", _Builder",
      "> {\n",
      "_typeAdapter\n",
      "}\n\n",
      "public static func serializer() -> SkirClient.Serializer<",
      selfTypeRef,
      "> {\n",
      "_ = ",
  ...
  private writeCodeForEnum(record: Record): void {
    ...
    this.push(
      ...
      "private static let _typeAdapter = SkirClient.EnumAdapter<",
      selfTypeRef,
      ">(\n",
      "modulePath: ",
    ...
    this.push(
      ...
      "}\n",
      ");\n\n",
      "fileprivate static func _adapterForInit() -> SkirClient.EnumAdapter<",
      selfTypeRef,
      "> {\n",
      "_typeAdapter\n",
      "}\n\n",
  ...
  private writeCodeForRecordSerializerInitialization(record: Record): void {
    ...
    if (record.recordType === "struct") {
      for (const field of record.fields) {
        ...
        this.push(`${typeRef}._adapterForInit().addField(\n`);
      ...
      for (const number of record.removedNumbers) {
        this.push(
          `${typeRef}._adapterForInit().addRemovedNumber(${number});\n`,
        );
      }
      this.push(`${typeRef}._adapterForInit().finalize();\n\n`);
      return;
    }
    ...
    for (let i = 0; i < variants.length; i++) {
      ...
      if (variant.type) {
        ...
        this.push(`${typeRef}._adapterForInit().addWrapperVariant(\n`);
        ...
        this.push(`${typeRef}._adapterForInit().addConstantVariant(\n`);
        this.push(`name: ${JSON.stringify(variant.name.text)},\n`);
        this.push(`number: ${variant.number},\n`);
        this.push(`kindOrdinal: ${kindOrdinal},\n`);
        this.push(`doc: ${JSON.stringify(docToCommentText(variant.doc))},\n`);
    ...
    for (const number of record.removedNumbers) {
      this.push(`${typeRef}._adapterForInit().addRemovedNumber(${number});\n`);
    }
    this.push(`${typeRef}._adapterForInit().finalize();\n\n`);
  }
