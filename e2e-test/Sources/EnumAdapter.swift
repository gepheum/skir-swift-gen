...
extension SkirClient.Internal {
    ...
    fileprivate struct ConstantEntry<T>: VariantEntry {
        ...
        func variantToJson(_ input: T, eolIndent: String?, out: inout String) {
            if eolIndent != nil {
                SkirClient.writeJsonEscapedString(variantName, out: &out)
            } else {
        ...
        func encodeValue(_ input: T, out: inout [UInt8]) {
            SkirClient.encodeUInt32(UInt32(bitPattern: variantNumber), out: &out)
        }

        func wrapFromJson(_ json: Any, keepUnrecognized: Bool) throws -> T {
            throw SkirClient.DeserializeError.schema("variant '\(variantName)' is a constant, not a wrapper")
        }
    ...
    fileprivate struct WrapperEntry<T, V>: VariantEntry {
        let variantNumber: Int32
        let variantName: String
        let serializer: SkirClient.Serializer<V>
        let wrap: (V) -> T
        let getValue: (T) -> V
    ...
    public final class EnumAdapter<T>: SkirClient.TypeAdapter {
        public typealias T = T

        private let getKindOrdinal: (T) -> Int
        private let wrapUnrecognized: (SkirClient.UnrecognizedVariantData<T>) -> T
        ...
        public init(
            ...
            wrapUnrecognized: @escaping (SkirClient.UnrecognizedVariantData<T>) -> T,
        ...
        public func addWrapperVariant<V>(
            ...
            number: Int32,
            kindOrdinal: Int,
            serializer: SkirClient.Serializer<V>,
            doc: String,
            wrap: @escaping (V) -> T,
                ...
                if let ko = nameToKindOrdinal[str] {
                    if ko < kindOrdinalToEntry.count, let entry = kindOrdinalToEntry[ko] {
                        if let constant = entry.constant() {
                            ...
                            throw SkirClient.DeserializeError.schema("variant '\(str)' is a wrapper, not a constant")
                        }
                    }
                }
                return defaultValue
            ...
            if let arr = json as? [Any], arr.count == 2 {
                ...
                case nil:
                    if keepUnrecognizedValues {
                        ...
                        return wrapUnrecognized(SkirClient.UnrecognizedVariantData(format: .denseJson, number: num, value: Array(bytes)))
                    }
                    return defaultValue
                case .removed:
                    return defaultValue
                case .constant:
                    throw SkirClient.DeserializeError.schema("variant number \(num) is a constant, not a wrapper")
                case .wrapper(let ko):
                    if ko < kindOrdinalToEntry.count, let entry = kindOrdinalToEntry[ko] {
                ...
                if let ko = nameToKindOrdinal[name] {
                    if ko < kindOrdinalToEntry.count, let entry = kindOrdinalToEntry[ko] {
                        if entry.constant() != nil {
                            throw SkirClient.DeserializeError.schema("variant '\(name)' is a constant, not a wrapper")
                        }
        ...
        private func resolveConstantLookup(_ number: Int32, keepUnrecognized: Bool, rawJson: Any) -> T {
            ...
            case nil:
                if keepUnrecognized {
                    ...
                    return wrapUnrecognized(SkirClient.UnrecognizedVariantData(format: .denseJson, number: number, value: bytes))
                }
                return defaultValue
            case .removed:
                return defaultValue
        ...
        public func decode(_ input: inout [UInt8], keepUnrecognizedValues: Bool) throws -> T {
            ...
            if wire < 242 {
                ...
                let number = Int32(truncatingIfNeeded: try SkirClient.decodeNumberBody(wire, input: &input))
                switch numberToEntry[number] {
                case nil:
                    if keepUnrecognizedValues && !removedNumbers.contains(number) {
                        ...
                        return wrapUnrecognized(
                            SkirClient.UnrecognizedVariantData(format: .bytes, number: number, value: consumed))
                    }
                    return defaultValue
                case .removed:
                    return defaultValue
